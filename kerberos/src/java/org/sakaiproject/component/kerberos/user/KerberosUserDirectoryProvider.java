...
public class KerberosUserDirectoryProvider implements UserDirectoryProvider
{
	...
	public void init()
	{
		...
		if (kerberoskrb5conf != null)
		{
			...
			{
				M_log.info(this + ".init(): Using default rules for krb5.conf location.");
		...
		if (kerberosauthloginconfig != null)
		{
			...
			{
				M_log.info(this + ".init(): Cannot set kerberosauthloginconfig location");
		...
		{
			throw new IllegalStateException("If you don't require local accounts you must set the domain for email addresses.");
		}
